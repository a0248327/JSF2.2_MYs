	...
	public void validate(FacesContext ctx, UIComponent comp, Object value) throws ValidatorException {
		if (value != null) {
			...
			if (minAge != null && MyGourmetUtil.getAge(date) < minAge) {
				FacesMessage msg = GuiUtil.getFacesMessage(ctx, FacesMessage.SEVERITY_ERROR, "validateAge.TOO_YOUNG", minAge);
				throw new ValidatorException(msg);
			}

			if (maxAge != null && MyGourmetUtil.getAge(date) > maxAge) {
				FacesMessage msg = GuiUtil.getFacesMessage(ctx, FacesMessage.SEVERITY_ERROR, "validateAge.TOO_OLD", maxAge);
				throw new ValidatorException(msg);
			}
		}
	}
