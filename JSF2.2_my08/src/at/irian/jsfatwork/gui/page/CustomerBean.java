...
public class CustomerBean {
	...
	private void init() {
		...
		SelectItem[] euItems = { new SelectItem("IT", getCategoryLabel("IT")), new SelectItem("GR", getCategoryLabel("GR")), new SelectItem("AT", getCategoryLabel("AT")) };
		SelectItemGroup euGroup = new SelectItemGroup("Europe", null, false, euItems);
	...
	public void validateCreditNumber(FacesContext ctx, UIComponent component, Object value) throws ValidatorException {
		...
		if (useCC != null && useCC && ccType != null) {
			...
			if (ccNumber != null && !ccNumber.matches("\\d{" + length + "}")) {
				FacesMessage msg = GuiUtil.getFacesMessage(ctx, FacesMessage.SEVERITY_ERROR, "validateCreditCardNumber.NUMBER", length);
